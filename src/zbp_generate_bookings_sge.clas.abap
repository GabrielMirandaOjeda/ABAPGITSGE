CLASS zbp_generate_bookings_sge DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zbp_generate_bookings_sge IMPLEMENTATION.

  METHOD if_oo_adt_classrun~main.
    DATA:it_bookings TYPE TABLE OF ztbooking_sge.
...
*   Delete the possible entries in the database table - in case it was already filled
    DELETE FROM ztbooking_sge.
*   insert the new table entries
    INSERT ztbooking_sge FROM TABLE @it_bookings.

*   check the result
    SELECT * FROM ztbooking_sge INTO TABLE @it_bookings.
    out->write( sy-dbcnt ).
